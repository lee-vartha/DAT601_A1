...
-- customer name, customer email, order entry ID, order item, rpoduct name, vendor name, vendor city
-- joining customer, order entry, product, vendor
-- condition is city isnt the same as vendor CustCity IS NOT VendorCity
...
-- TODDS PRACTICE TEST QUERY
SELECT Customer.CustName, Customer.CustEmail, OrderEntry.OrderID, OrderEntry.OrderDate, OrderItem.OrderItem, Product.ProductID, Vendor.VendorName, Vendor.VendorCity
FROM Customer
JOIN OrderEntry ON OrderEntry.CustID = Customer.CustID
JOIN OrderItem ON OrderItem.OrderID = OrderEntry.OrderID
JOIN Product ON OrderItem.ProductID = Product.ProductID
JOIN Vendor ON Product.VendorID = Vendor.VendorID
WHERE Customer.CustCity <> Vendor.VendorCity;